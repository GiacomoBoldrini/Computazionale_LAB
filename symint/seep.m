...
%  [Q P] = SEEP(DQDT,DPDT,TSPAN,Q0,P0) solves Hamilton's equations dq/dt = 
...
%
%  [Q P] = SEEP(DQDT,DPDT,TSPAN,Q0,P0,VARARGIN) passes the additional
%  parameters R1, R2, ... to functions DQDT(P,R1,R2,...) and DPDT(Q,R1,
%  R2,...).
%
%  To construct this numerical integrator the p-variable is treated by the
%  implicit Euler method and the q-variable by the explicit Euler method 
...
for nt = 2:Nt
   ...
   h = hs(nt-1);
	p(:,nt) = p(:,nt-1) + h * feval(dpdt, q(:,nt-1), varargin{:}); 
   q(:,nt) = q(:,nt-1) + h * feval(dqdt, p(:,nt)  , varargin{:});
end
q = q.';
p = p.';